...
class ArgTypeExpander:
    ...
    **kwargs, and to keep track of which tuple/TypedDict items have already been
    ...
    def expand_actual_type(
        ...
        If the actual argument is a tuple *args, where the caller expects a positional,
        return the next individual tuple item that maps to the formal arg.
        ...
        if actual_kind == nodes.ARG_STAR:
            # the caller provided a *args argument, args can be any iterable type.
            # if formal_kind == nodes.ARG_STAR:
            #     # the function expects a *args argument, so convert to
            #     return actual_type

            # parse actual_type as one of the following:
            #    TupleInstanceType | TupleType | ParamSpecType | AnyType
            star_args_type = self.parse_star_args_type(actual_type)
            # TODO: DO DIFFERENT THINGS DEPENDING ON FORMAL_KIND.
            if formal_kind == nodes.ARG_STAR:
                if isinstance(star_args_type, ParamSpecType | AnyType):
                    return star_args_type
                # we always treat *args as expecting an UnpackType.
                return UnpackType(star_args_type)
            elif formal_kind in (nodes.ARG_POS, nodes.ARG_OPT):
                if self.context.is_iterable_instance_type(star_args_type):
